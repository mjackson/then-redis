...
// Set the client's database property to the database number when SELECTing.
addCommand('select', function (database) {
  var self = this;
...
addCommand('hgetall', function () {
  return this.send('hgetall', _slice.call(arguments, 0)).then(function (array) {
    var hash = {};
...
// Optionally accept a hash as the only argument to HMSET.
addCommand('hmset', function (key, hash) {
  var args;
  if (typeof hash === 'object') {
    args = [ key ];
    for (var field in hash) {
      if (hash.hasOwnProperty(field)) {
        args.push(field, hash[field]);
      }
    }
  } else {
