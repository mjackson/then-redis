...
Client.prototype._handleReply = function (reply) {
  ...
  var promise = this._commandPromises.shift();
...
Client.prototype._flushError = function (error) {
  var promise;
  while (promise = this._commandPromises.shift()) {
...
Client.prototype._flushPendingWrites = function () {
  var write;
  while (write = this._pendingWrites.shift()) {
...
Client.prototype.connect = function () {
  if (!this._connectPromise) {
    ...
    connection.on('connect', function () {
      ...
      var authPromise = self.password ? self.auth(self.password) : resolve('OK');
      var selectPromise = self.database ? self.select(self.database) : resolve('OK');
      self._flushPendingWrites();

      return rsvp.all([ authPromise, selectPromise ]).then(function (values) {
        self._connectPromise.resolve(values);
      }, function (error) {
        self._connectPromise.reject(error);
      });
    });
...
Client.prototype.send = function (command, args) {
  ...
  this._commandPromises.push(promise);
