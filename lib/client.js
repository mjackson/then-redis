...
Client.prototype.connect = function () {
  if (!this._connectValue) {
    ...
    connection.on('connect', function () {
      ...
      var authPromise = self.password ? self.auth(self.password) : when.resolve('OK');
      var selectPromise = self.database ? self.select(self.database) : when.resolve('OK');
      self._flushPendingWrites();

      self._connectValue.resolve(when.all([ authPromise, selectPromise ]));
    });
